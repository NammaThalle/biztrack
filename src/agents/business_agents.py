...
class IntentDetectionAgent(Agent):
    ...
    def detect_intent(self, message, context=None):
        ...
        prompt = (
            ...
            "- If the user asks about products, vendors, customers, transactions, or business data, use 'graph_query'\n"
            "- If the user says 'add', 'create', 'new', 'insert' followed by product/vendor/customer, use 'graph_query'\n"
            "- If the user says 'Okay lets add one more then' or similar, they want to add a product - use 'graph_query'\n"
            "- If the user asks 'What products do we have?' or similar, use 'graph_query'\n"
            "- If the user asks 'Anything else?' or 'Do we have anything else?', use 'graph_query' to check the database\n"
            "- Only use 'chat' for general greetings, casual conversation, or when you need to ask for more information\n"
            "\n"
